...
I want to explore the new variables, age and eye color
This cell will plot Age as a histogram
```{r}
p5 <- mydata %>% ggplot(aes(x=Age)) + geom_histogram()
plot(p5)
figure_file = here("results", "figures", "AgeHistogram.png")
ggsave(filename = figure_file, plot=p5)
```
I can see that only one age repeats, and that this dataset is a bunch of older adults.

Now to map weight and age in a scatter plot
```{r}
p6 <- mydata %>% ggplot(aes(x=Weight, y=Age)) + geom_point()
plot(p6)
figure_file = here("results", "figures", "AgeWeight.png")
ggsave(filename = figure_file, plot=p6)
```
It looks like there is maybe a positive correlation between Age and weight

Now I want to make a box plot graphing eye color by height
```{r}
p7 <- mydata %>% ggplot(aes(x=`Eye Color`, y=Height)) + geom_boxplot()
plot(p7)
figure_file = here("results", "figures", "HeightEyeColor.png")
ggsave(filename = figure_file, plot=p7)
```
It looks like people with brown eyes tend to be shorter which is interesting!
